(function ($) {
  var self = Drupal.behaviors.contenteditable = {
    attach: function (context) {
      ...
      // Bind handlers and prevent elements from being processed
      //again.
    ...
    init: function () {
      ...
      var $controls = self.constructControls().add('<button>', {
        'id':
          'contenteditableCancel', 'text': Drupal.t('Cancel'), click:
        self.hideControls, 'data-tooltip': 'cancel edit'
      })
        .add('<button>', {
          'id': 'contenteditableSave', 'text':
            Drupal.t('Save'), click: self.submitHandler, 'data-tooltip':
            'save changes'
        });
      self.controls = $('<div id="contenteditableButtons" class="contenteditable_buttons"></div>').append($controls)
        .appendTo('body');
    ...
    focusin: function (e) {
      ...
      //but only when focusing in on a new field, otherwise do
      //nothing.
    ...
    constructControls: function () {
      ...
      // Go over the settings object, construct the controls and
      //return them as 1 jQuery collection.
      var $buttons = $();
...
//edited
    ...
    hideControls: function () {
      ...
      var $clone = self.controls
        .removeAttr('style').clone(true).appendTo('body');
      self.controls.remove();
      self.controls = $clone;
